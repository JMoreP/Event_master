...
const CreateSpeaker = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Por ahora regresamos al panel, luego idealmente a la lista de ponentes
            showToast('Ponente guardado con éxito.', 'success');
            navigate('/panel');
        } catch (error) {
            console.error('Error al guardar ponente:', error);
            showToast('Hubo un error al guardar el ponente.', 'error');
        } finally {
            setUploading(false);
        }
    };
    ...
    return (
        <div className="flex flex-col max-w-[800px] w-full mx-auto px-6 py-8">
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-6">Nuevo Ponente</h2>
            ...
            <div className="bg-surface-light dark:bg-surface-dark rounded-xl p-6 md:p-8 shadow-sm border border-slate-200 dark:border-slate-800">
                <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
                    ...
                    <div className="flex justify-end gap-3 pt-6 border-t border-slate-200 dark:border-slate-700">
                        ...
                        >
                            {uploading ? 'Guardando...' : 'Guardar Ponente'}
                        </button>
                    </div>
                </form>
            </div>
